...
module.exports = (db) => {
  ...
  const getOrderInfo = function (user_id) {
    return db.query(`
    SELECT
    users.id AS user_id,
    order_items.food_order_id AS order_number,
    food_orders.created_at,
    food_orders.comments,
    food_orders.status,
    order_items.menu_item_id,
    menu_items.title,
    menu_items.price,
    menu_items.prep_time as prep_time
  FROM
    order_items
  ...
  WHERE
    users.id = 1
    GROUP BY users.id, menu_items.price, menu_items.prep_time, menu_items.title, order_items.menu_item_id, food_orders.status, food_orders.comments, order_items.food_order_id, food_orders.created_at
